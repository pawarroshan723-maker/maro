...
const fn = new Function('document','window','navigator','localStorage','performance','requestAnimationFrame','cancelAnimationFrame','screen',
...
console.log('onGround:', P.onGround, 'y:', P.y, 'h:', P.h, 'form:', P.form);
// direct call
Game.onAction(P);
console.log('after direct onAction: shots=', Game.shots.length);
// via input queue
Input.actionQueued = true;
console.log('queued before frame:', Input.actionQueued);
pump(1);
console.log('after 1 frame: queued=', Input.actionQueued, 'shots=', Game.shots.length);
// single step trace of p.update
Input.actionQueued = true;
try { P.update(1/60); console.log('manual update ok, shots=', Game.shots.length); } catch(e){ console.log('P.update threw:', e.message); }
process.exit(0);

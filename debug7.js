...
console.log('pre-jump: x=', P.x.toFixed(0), 'y=', P.y.toFixed(1), 'onGround=', P.onGround, 'crouch=', P.crouching, 'vx=', P.vx.toFixed(1));
Input.jumpHeld = true; Input.jumpQueued = true;
for (let i=0;i<12;i++){
  nowMs += 1000/60; const cbs = rafCbs.splice(0); for (const cb of cbs) cb(nowMs);
  console.log(`f${i}: y=${P.y.toFixed(1)} vy=${P.vy.toFixed(1)} onGround=${P.onGround} jbuf=${P.jbuf.toFixed(3)} coyote=${P.coyote.toFixed(3)} state=${Game.state}`);
}
process.exit(0);

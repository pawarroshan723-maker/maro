...
const tag = (s) => console.log(s, 'lives=', Game.lives, 'state=', Game.state, 'x=', Game.player.x.toFixed(0), 'y=', Game.player.y.toFixed(0));

Game.startGame(); pump(90);
Input.right = true; pump(80); Input.right = false; pump(30);
tag('after walk');
// block bump
teleport(7*48 + 10, 10*48 - Game.player.h);
Game.player.vy = -560; Input.jumpHeld = true; pump(30); Input.jumpHeld = false; pump(40);
tag('after block bump');
// stomp
...
pump(60);
tag('60f later');
// what walkers are alive & where
Game.enemies.forEach((en,i) => { if (!en.remove) console.log('  enemy', i, en.kind, 'x=', en.x.toFixed(0), 'active=', en.active, 'dead=', en.dead); });
process.exit(0);
